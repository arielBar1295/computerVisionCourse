...
def blendDemo():
    ...
    #my blending
    img1_1=cv2.cvtColor(cv2.imread('input/uni.jpg'), cv2.COLOR_BGR2RGB) / 255
    img2_2=cv2.cvtColor(cv2.imread('input/doll.png'), cv2.COLOR_BGR2RGB) / 255
    mask2= cv2.cvtColor(cv2.imread('input/dollMask.png'), cv2.COLOR_BGR2RGB) / 255
    img1_1 = cv2.resize(img1_1,(img2_2.shape[1],img2_2.shape[0]))
    n_blend, im_blend = pyrBlend(img2_2,img1_1, mask2, 4)

    f, ax = plt.subplots(2, 3)
    ax[0, 0].imshow(img1_1)
    ax[0, 1].imshow(img2_2)
    ax[0, 2].imshow(mask2)
    ax[1, 0].imshow(n_blend)
    ax[1, 1].imshow(np.abs(n_blend - im_blend))
    ax[1, 2].imshow(im_blend)
...
def main():
    ...
    pyrLaplacianDemo('input/phoo.png')
    blendDemo()
